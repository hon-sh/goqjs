...
Same Vite IIFE (`dist/server/ssr.js`) for **Bun, Deno, and hon** so React work is identical.
...
cd bench/ssr
npm install
npm run build          # hon IIFE → dist/server/ssr.js
```
...
```bash
make -C bench ssr
# DENO_PARALLEL=1 HON_C=8 make -C bench ssr
# or: ./bench/ssr/run.sh
```

Order: bun → deno → hon `-c 1` → hon `-c $nproc`. Outputs under `bench/results/ssr-<stamp>.*` (gitignored). Promote a snapshot into [`../BENCH-ssr.md`](../BENCH-ssr.md) after review.
...
Env overrides: `LATENCY_N`, `LATENCY_DELAY`, `CONCUR_N`, `CONCUR_DELAY`, `RENDER_N`, `RENDER_DELAY`, `HON_C`, `DENO_PARALLEL`, `PORT`.
...
| `src/entry-hon.jsx` | sets `globalThis.__bench_render` |
| `dist/server/ssr.js` | Vite IIFE (Bun / Deno / hon all load this) |
| `server_bun.js` / `server_deno.js` | HTTP hosts (`eval` IIFE) |
| `handler.js` + `main.go` | hon pool host |
| `run.sh` | sequential orchestrator |

## Why not hn-ssr / live fetch?

- HN Firebase adds **network jitter** and API coupling.
- This bench keeps delay **deterministic** so Bun / Deno / hon compare event-loop + render, not WAN latency.
