...
| `.bin/` | local `gohon-serve` / `bench-ssr` binaries (gitignored) |
...
hon 追不上 bun/deno，主因是 **QuickJS 无 JIT**，不是 Go 侧一把大锁，也不是 bun/deno 默认就把每个请求丢到多条 JS 线程。单请求变快要靠引擎；多核吞吐要靠 **`-c N` 多 Runtime**。
...
一句话：hon 适合 **Go 宿主 + 中等 React 渲染 + 大量 await I/O**，尖峰用池；不适合指望单 isolate 硬扛超大 `renderToString`，更别用 fib 当 SSR 成绩单。
