...
| `serve-fib.js` | goqjs-serve handler (keep in sync with `fib.js`) |
| `run.sh` | sequential orchestrator + version gate |

Demo copy for `goqjs-serve` CLI docs: `examples/serve-fib.js` (same algorithm).
...
- For multi-core CPU passes, set `DENO_PARALLEL=1`; default single worker is closer to goqjs `-c 1`.
